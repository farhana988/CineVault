...
import { sortMovies } from "@/helper/sortMovies";
import { GenreTabsProps, MovieSortOption, SortOrder } from "@/types/movie.type";
...
const GenreTabs = ({ moviesByGenre }: GenreTabsProps) => {
  ...
  const [sortBy, setSortBy] = useState<MovieSortOption>("popularity");

  const [order, setOrder] = useState<SortOrder>("desc");

  const sortedMoviesByGenre = moviesByGenre.map((group) => ({
    ...group,
    movies: sortMovies(group.movies, sortBy, order),
  }));

  const selectedGenre = sortedMoviesByGenre.find(
